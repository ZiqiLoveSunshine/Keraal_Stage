...
def batch_copy(source_path, suffix, dest_path, dest_suffix=None):
    ...
    for f in files:
        if dest_suffix is not None:
            shutil.copyfile(os.path.join(source_path, f), os.path.join(dest_path, f[:-length] + '_{}.bvh'.format(dest_suffix)))
            # cmd = 'cp \"{}\" \"{}\"'.format(os.path.join(source_path, f), os.path.join(dest_path, f[:-length] + '_{}.bvh'.format(dest_suffix)))
        else:
            shutil.copyfile(os.path.join(source_path, f), os.path.join(dest_path, f[:-length] + '.bvh'))

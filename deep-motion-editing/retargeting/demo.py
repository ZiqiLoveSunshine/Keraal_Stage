...
def example_different_file(src_name, dest_name, test_bvh_name, target_bvh_name, test_type, output_path):
    if not os.path.exists(output_path):
        os.makedirs(output_path)

    input_file = './datasets/Mixamo/{}/{}'.format(src_name, test_bvh_name)
    ref_file = './datasets/Mixamo/{}/{}'.format(dest_name, target_bvh_name)
    copy_ref_file(input_file, pjoin(output_path, 'input.bvh'))
    copy_ref_file(ref_file, pjoin(output_path, 'gt.bvh'))
    height = get_height(input_file)

    test_bvh_name = test_bvh_name.replace(' ', '_')
    target_bvh_name = target_bvh_name.replace(' ', '_')
    input_file = './datasets/Mixamo/{}/{}'.format(src_name, test_bvh_name)
    ref_file = './datasets/Mixamo/{}/{}'.format(dest_name, target_bvh_name)
...
if __name__ == '__main__':
    ...
    example_different_file('Aj', 'BigVegas', 'Dancing Running Man.bvh', 'Dual Weapon Combo.bvh', 'cross', './examples/cross_structure')
    print('Finished!')

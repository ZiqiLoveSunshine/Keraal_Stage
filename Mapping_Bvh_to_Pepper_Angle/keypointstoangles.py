...
class KeypointsToAngles(object):
    ...
    # 
    #   Calculate Left wrist yaw angles
    def obatin_LwristYaw_angle(self, P5, P6, P7, P9, P11):
     # Construct 3D vectors (bones) from points
        v_6_5 = self.vector_from_points(P6, P5)
        v_6_7 = self.vector_from_points(P6, P7)
        v_7_9 = self.vector_from_points(P7, P9)
        v_7_11 = self.vector_from_points(P7, P11)

        n_5_6_7 = np.cross(v_6_5, v_6_7)
        n_7_9_11 = np.cross(v_7_9, v_7_11)

        x = np.dot(n_5_6_7, n_7_9_11) / (np.linalg.norm(n_5_6_7))*(np.linalg.norm(n_7_9_11))
        # print(x)
        try:
            LwristYaw = math.acos(x)
        except ValueError:
            LwristYaw = 0
    ...
    # 
    #   Calculate Right wrist yaw angles
    def obatin_RwristYaw_angle(self, P5, P6, P7, P10, P12):
     # Construct 3D vectors (bones) from points
        v_6_5 = self.vector_from_points(P6, P5)
        v_6_7 = self.vector_from_points(P6, P7)
        v_7_10 = self.vector_from_points(P7, P10)
        v_7_12 = self.vector_from_points(P7, P12)

        n_5_6_7 = np.cross(v_6_5, v_6_7)
        n_7_10_12 = np.cross(v_7_10, v_7_12)

        x = np.dot(n_5_6_7, n_7_10_12) / (np.linalg.norm(n_5_6_7))*(np.linalg.norm(n_7_10_12))
        try:
            RwristYaw = -math.acos(x)
        except ValueError:
            RwristYaw = 0
            
        return RwristYaw

    # ##----------------    obatin_HeadPitch_angle   -------------------------
    # # 
    # #   Calculate head pitch angles
    # def obatin_HeadPitch_angle(self, P0, P1):
    #  # Construct 3D vectors (bones) from points
    #     v_0_1 = self.vector_from_points(P0, P1)
    #     h = np.linalg.norm(v_0_1)
    #     P_0_1 = abs(P0[2] - P1[2])

    #     if (P_0_1 / h) >= 0.95 and (P_0_1 / h) <= 1.0:
    #         HeadPitch = 0.0

    #     elif P_0_1 / h < 0.95:
    #         HeadPitch = ((h - P_0_1) / h) * 0.637
    #         if HeadPitch >= 0.637:
    #             HeadPitch = 0.637

    #     elif P_0_1 / h > 1.0:
    #         HeadPitch = -((P_0_1) / h) * 0.7068 * 2
    #         if HeadPitch <= -0.7068:
    #             HeadPitch = -0.7068
    #     # HeadPitch = 0.0    
    #     return HeadPitch
    ...
    # # 
    # #   Calculate head yaw angles
    # def obatin_HeadYaw_angle(self, P0, P2, P5):
    #  # Construct 3D vectors (bones) from points
    #     v_2_5 = self.vector_from_points(P2, P5)
    #     d = np.linalg.norm(v_2_5) / 2
    #     P_0_2 = abs(P0[2] - P2[2])

    #     if P_0_2 / d >= 0.9 and P_0_2 / d <= 1.1:
    #         HeadYaw = 0.0

    #     elif P_0_2 / d < 0.9:
    #         HeadYaw = -((d - P_0_2) / d) * np.pi / 2
    #         if HeadYaw <= -np.pi / 2:
    #             HeadYaw = -np.pi / 2

    #     elif P_0_2 / d > 1.1:
    #         HeadYaw = ((P_0_2 - d) / d) * np.pi / 2
    #         if HeadYaw >= np.pi / 2:
    #             HeadYaw = np.pi / 2
    #     HeadYaw = 0.0    
    #     return HeadYaw
    ...
    def obtain_LElbowYawRoll_angle(self, P1, P5, P6, P7):
        ...
        LHand = 0.5*math.cos(LElbowRoll) + 0.5
        return LElbowYaw, LElbowRoll, LHand
    ...
    def obtain_RElbowYawRoll_angle(self, P1, P2, P3, P4):
        ...
        RHand = 0.5*math.cos(RElbowRoll) + 0.5
        return RElbowYaw, RElbowRoll, RHand
